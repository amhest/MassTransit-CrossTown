...
// specific language governing permissions and limitations under the License.
    ...
    public class InMemoryInboundMessageTracker implements
        IInboundMessageTracker
    {
        final ConcurrentMap<String, TrackedMessage> _messages;
        final int _retryLimit;

        public InMemoryInboundMessageTracker(int retryLimit)
        {
            _retryLimit = retryLimit;

            _messages = new ConcurrentHashMap(String, TrackedMessage>(id => new TrackedMessage());
        }

        public boolean IsRetryLimitExceeded(string id, out Exception retryException, out IEnumerable<Action> faultActions)
        {
            bool exceeded = false;
